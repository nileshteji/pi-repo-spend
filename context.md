...
- `index.ts`: Pi extension entry point; scans Pi JSONL session logs, aggregates token usage and costs by cwd/repo/provider/model, adds monthly and daily buckets for exact-cwd reports, estimates Ollama Cloud costs with hardcoded pricing, and exposes `/repo-spend`. The default renderer is a graphical Pi TUI dashboard; `text` mode renders the copyable Markdown report. It intentionally does not keep a footer/status spend indicator.
